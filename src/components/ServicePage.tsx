...
export default function ServicePage({ title, description, benefits }: ServicePageProps) {
  return (
    <main className="bg-slate-950 pt-28 text-white sm:pt-36">
      <section className="border-b border-white/5 px-4 pb-16 sm:px-6 sm:pb-20 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_410px] lg:items-center">
          <div>
            ...
            <ul className="mt-8 grid gap-3 sm:grid-cols-2">
              {benefits.map((benefit) => (
                <li key={benefit} className="flex items-center gap-2.5 text-sm text-slate-200">
                  <CheckCircle2 className="size-5 shrink-0 text-orange-400" />
                  {benefit}
                </li>
              ))}
            </ul>
          </div>
          <CarEvaluationForm title="Дізнайтесь вартість авто" />
        </div>
      </section>
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-5 md:grid-cols-3">
          <Info
            ...
            title="Швидко"
            text="Оцінка по фото за 5 хвилин і оформлення в день звернення."
          />
          <Info
            icon={<Truck className="size-5" />}
            title="Безкоштовно"
            text="Виїзд фахівця та евакуатор по Харкову й області."
          />
          <Info
            icon={<ShieldCheck className="size-5" />}
            title="Безпечно"
            text="Прозорий договір та повний розрахунок на місці."
          />
        </div>
      </section>
    </main>
